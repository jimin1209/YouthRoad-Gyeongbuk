################################################################################
# BLOCK A. 1차 릴리즈 전에 반드시 처리할 핵심 기능 TASK
################################################################################
...
TASK_COMPARE_ZOOM_FIX:
  ...
  non_goals:
    - GPS 기반 자동 '내 지역 추론' 기능은 이번 작업에 포함하지 않는다.
    - 읍면동 단위의 상세 지역 선택은 별도 TASK에서 다룬다.

  acceptance_criteria:
    - [ ] 탐색 탭 상단에서 '경북 전체 ▼' 콤보박스를 통해
          시·군을 명시적으로 선택할 수 있다.
    - [ ] 선택한 지역이 필터 요약 문구 및 실제 정책 목록에 반영된다.
    - [ ] 기존의 '내 지역' 버튼이 더 이상 UI에 표시되지 않는다.
